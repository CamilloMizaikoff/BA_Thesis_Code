...
def almeida_circadian_clock(t, r):
	...
	#E_box = (V_E * BMAL1)/(BMAL1 + k_E + (k_Er * BMAL1 * CRY))
	#RRE = ((V_R * ROR)/(ROR + k_R)) * ((k_Rr**2)/(k_Rr**2 + REV**2))
	#D_box = ((DBP)/(DBP + k_D)) * ((k_Dr)/(k_Dr + E4BP4))
	E_box = (V_E_m * BMAL1)/(BMAL1 + k_E_m + k_Er_m * BMAL1 * CRY + fac * BMAL1 * PER_CRY)
